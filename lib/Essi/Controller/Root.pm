...
sub _build {
  ...
  ## Build
  `export DEB_BUILD_OPTIONS=nocheck && cd /tmp/essi_$guid/repo && perl Makefile.PL && dh-make-perl -vcs none $depends && dpkg-buildpackage -d -us -uc && cp /tmp/essi_$guid/*.deb $deb_path && cp /tmp/essi_$guid/*.changes $deb_path && cp /tmp/essi_$guid/*.dsc $deb_path && cp /tmp/essi_$guid/*.tar.xz $deb_path && cp /tmp/essi_$guid/*.tar.gz $deb_path && rm -rf /tmp/essi_$guid/`;
